function Update-OPDscLcmConfigurationFile {
    ...
    Function Update-ConfigurationValue {
        ...
        foreach($subKey in $Item.Keys) {
            try{
                $typeFullName = $Item[0].$subKey.GetType().FullName
            }
            catch {
                Write-Verbose $_
            }
    ...
    if($SectionUpdatePreference -eq 'Merge') {
        $setNewValueScript = [System.Text.StringBuilder]::new()
    }

    foreach ($key in $Sections.Keys.Where{ $_ -ne 'AllNodes' }) {
        if(-not $updateSections.ContainsKey($key) -or $SectionUpdatePreference -eq 'Replace') {
            $updateSections[$key] = $Sections.$key
        }
        elseif($SectionUpdatePreference -eq 'Merge') {
            $null = $setNewValueScript.AppendLine(
                $(
    ...
    if ($setNewValueScript.Length) {
        [scriptblock]::Create($setNewValueScript.ToString()).Invoke()
    }
